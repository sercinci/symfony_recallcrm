 ...
 */
namespace PairPI\Bundle\RecallcrmBundle;
...
class AfterLoginRedirection implements AuthenticationSuccessHandlerInterface
{
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token)
    {
        ...
        if (in_array('ROLE_ADMIN', $rolesTab, true) || in_array('ROLE_SUPER_ADMIN', $rolesTab, true))
            $redirection = new RedirectResponse($this->router->generate('report'));
        // otherwise, if is a commercial user we redirect to the crm area
        elseif (in_array('ROLE_OPERATORE', $rolesTab, true))
            $redirection = new RedirectResponse($this->router->generate('campagne'));
